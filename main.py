...
class Patient(BaseModel):
    ...
    name : Annotated[str, Field(..., description = 'Name of the patient',examples=['Uday'])]
...
def load_data():
    with open('patients_api_format.json','r') as f:
        data = json.load(f)
    
    return data

def save_data(data):
    with open('patients_api_format.json','w') as f:
...
def patient(patient_id:str = Path(..., description='ID of the patient in the DB', examples=['P001'])):
    data = load_data()
    if(patient_id in data):
        return data[patient_id]
    raise HTTPException(status_code=404, detail='Patient not found')
    # return {'error: patient_id is not in the data'}  #showing status code as 200 even though data not found 
...
def create_patient(patient:Patient):
    ...
    #check if the existing patient already exists
...
def update_patient(patient_id:str , patient_update:PatientUpdate):
    ...
    return JSONResponse(status_code=200,content={'message':'patient_update'})
